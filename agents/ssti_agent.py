...
class SSTIExpertAgent:
    ...
    def run(self, target, instruction):
        prompt = (
            "You are a web exploitation expert specializing in SSTI (Server-Side Template Injection).\n"
            "You have been tasked with exploiting a specific page or input based on the following instruction:\n\n"
            f"{instruction}\n\n"
            "You should:\n"
            "- Use your expert knowledge and available tools to test for SSTI vulnerabilities\n"
            ...
            "Do not perform actions outside the provided instruction. Stay focused on SSTI.\n\n"
            f"Target: {target}"
        )
    ...
    def node(self, state):
        ...
        instruction = state.get("instruction", f"Test SSTI vulnerabilities on {target_url}")
        ...
        # Update messages
        messages = state.get("messages", []) + [HumanMessage(content=f"SSTI result: {result}")]
