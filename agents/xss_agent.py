...
class XSSExpertAgent:
    ...
    def run(self, target, instruction):
        prompt = (
            "You are a web exploitation expert specializing in XSS (Cross-Site Scripting).\n"
            "You have been tasked with exploiting a specific page or input based on the following instruction:\n\n"
            f"{instruction}\n\n"
            "You should:\n"
            "- Use your expert knowledge and available tools to test for XSS vulnerabilities\n"
            ...
            "Do not perform actions outside the provided instruction. Stay focused on XSS.\n\n"
            f"Target: {target}"
        )
    ...
    def node(self, state):
        ...
        instruction = state.get("instruction", f"Test XSS vulnerabilities on {target_url}")
        ...
        # Update messages
        messages = state.get("messages", []) + [HumanMessage(content=f"XSS result: {result}")]
